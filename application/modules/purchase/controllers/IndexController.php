...
class Purchase_indexController extends Zend_Controller_Action {
	...
	protected function ActionPurchaseAction() {
		...
		if ($this->getRequest ()->isPost ()) {
			...
			// $update_payment_order = new
			// purchase_Model_DbTable_DbPurchaseVendor();
			...
			// }
			// else{
			// Application_Form_FrmMessage::message("Cann't Edit!Purchase Order
			// Has Been Payment Already");
			// Application_Form_FrmMessage::redirectUrl("/purchase/index/index");
			// }
			$update_order = new purchase_Model_DbTable_DbPurchaseVendor ();
		...
		Application_Model_Decorator::removeAllDecorator ( $formpurchase_info ); // omit
		                                                                        // default
		                                                                        // zend
		                                                                        // html
		                                                                        // tag
	...
	public function getpobyidAction() {
		if ($this->getRequest ()->isPost ()) {
			...
			$row = $db->getGlobalDbRow ( $sql );
			/*
			 * if(!$result){ $result = array('contact'=>'','phone'=>''); }
			 */
			echo Zend_Json::encode ( $row );
			exit ();
		}
